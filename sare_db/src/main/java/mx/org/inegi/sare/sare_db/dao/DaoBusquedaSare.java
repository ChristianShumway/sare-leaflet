...
//    private String schemaocl;
    ...
    {
        EntityManager manager = DaoBusquedaSare.getEntityManager();
        //TypedQuery<TdInmuebles> consulta = manager.createQuery("select t from TdInmuebles t", TdInmuebles.class);
        TypedQuery<TdInmuebles> consulta = (TypedQuery<TdInmuebles>) manager.createNamedQuery("TdInmuebles.findAll");
        List<TdInmuebles> lista =  consulta.setMaxResults(5).getResultList();
    ...
    public List<TrEtqVal> busquedaPredios() 
    {
        ...
        Hibernate.initialize(lista.get(0).getTipoE14().getCveTipoAsen());
        manager.close();
        return lista;
    }
    ...
    public Short busquedaCve() {
        ...
        Hibernate.initialize(lista.get(0).getTipoE14().getCveTipoAsen());
        manager.close();
        return lista.get(0).getTipoE14().getCveTipoAsen();
    ...
    public List<cat_vw_punteo_sare> busqueda(Integer proyecto, String tramo, String ce, String usuario, int origen, String id_ue) {
        ...
        sql = getSql(null, 0, "", null, "", proyectos, ce, id_ue, origen, MetodosBusqueda.BUSQUEDAOCL, tramo);
        switch (proyectos) {
            case MasivoOtros:
    ...
    public ArrayList<String> getClavesUnicasPG(Integer proyecto) {
        ...
        StringBuilder sql;
        proyectos = getProyecto(proyecto);
        sql = getSql(null, 0, "", null, "", proyectos, "", "", 0, MetodosBusqueda.GETCLAVESPG, null);
    ...
    public List<cat_vw_punteo_sare> getDatosInmuebles(Integer proyecto, String id_ue) {
        ...
        StringBuilder sql;
        proyectos = getProyecto(proyecto);
        sql = getSql(null, 0, "", null, "", proyectos, "", id_ue, 0, MetodosBusqueda.GETDATOSINMUEBLES, null);
    ...
    public String getNombreBusqueda(Integer proyecto, Integer tipo, String tabla) {
        ...
        regresa = jdbcTemplate.query(sql.toString(), new Object[]{parametro}, new ResultSetExtractor<String>() {
            @Override
            public String extractData(ResultSet rs) throws SQLException, DataAccessException {
                String fila = null;
                while (rs.next()) {
    ...
    public String getNombreBusqueda(cat_vw_punteo_sare cat_vw_punteo_sare, Integer proyecto, int params, String campo, String tabla) {
        ...
        StringBuilder sql;
        proyectos = getProyecto(proyecto);
        sql = getSql(cat_vw_punteo_sare, params, tabla, null, campo, proyectos, "", "", 0, MetodosBusqueda.GETNOMBREBUSQUEDAOCL, null);
    ...
    public boolean getValCoorGeo(Integer proyecto, String x, String y, cat_vw_punteo_sare element) {
        boolean regresa = false;
        StringBuilder sql;
        cat_vw_punteo_sare object = new cat_vw_punteo_sare();
        proyectos = getProyecto(proyecto);
        ...
        object = jdbcTemplate.query(sql.toString(), new Object[]{x, y}, new ResultSetExtractor<cat_vw_punteo_sare>() {
            ...
            public cat_vw_punteo_sare extractData(ResultSet rs) throws SQLException, DataAccessException {
                boolean fila = false;
                cat_vw_punteo_sare element = new cat_vw_punteo_sare();
                while (rs.next()) {
                    fila = rs.getBoolean("error");
                    if (!fila) {
                        element.setCOORD_X(new BigDecimal(rs.getString("x_geo")));
                        element.setCOORD_Y(new BigDecimal(rs.getString("y_geo")));
                    }
                }
                return element;
            }
        });
        element.setCOORD_X(object.getCOORD_X());
        element.setCOORD_Y(object.getCOORD_Y());
        regresa = !(String.valueOf(object.getCOORD_X()) == null && String.valueOf(object.getCOORD_Y()) == null);
        return regresa;
    }
    ...
    public String getExtentBusquedaCvegeo(cat_vw_punteo_sare cat_vw_punteo_sare, Integer proyecto, int params, String tabla, boolean mza800, String[] rural) {
        ...
        StringBuilder sql;
        proyectos = getProyecto(proyecto);
        sql = getSql(cat_vw_punteo_sare, null, "", null, "", proyectos, "", "", 0, MetodosBusqueda.GETEXTENTCVEGEO, null);
            ...
            case Transportes:
                switch (metodo) {
                    ...
                    case GETNOMBREBUSQUEDAOCL:
                        ...
                        break;
                    case LIBERACLAVEUNICAORACLE:
                        sql.append("UPDATE ").append(esquemaOcl).append(".TR_UE_SUC set SARE_ST='10' where id_ue=? and sare_st<>'01'");
                        break;
                    case GETVALCOORGEO:
                        sql.append("select x_geo::varchar,y_geo::varchar,error from ").append(esquemaPos).append(".val_coordenada_geo(?,?) valida");
                        break;
    ...
    private StringBuilder filtrarSqlconglomerados(String ce, String esquemaPos, String id_ue, int origen, String tramo) {
        String esquemaOcl = esquemaPos;
        StringBuilder sql = new StringBuilder();
//        sql.append("SELECT to_char(id_ue) as id_ue, e03, e04, e05, e06, e07, e08, e09, lpad(to_char(tipo_e10),2,'0') tipo_e10, e10, e11, TRIM(e11a) as e11a, lpad(to_char(tipo_e14),2,'0') tipo_e14, e14, lpad(to_char(tipo_e10_a),2,'0') tipo_e10_a, e10_a, ");
//        sql.append("lpad(to_char(tipo_e10_b),2,'0') tipo_e10_b, e10_b, lpad(to_char(tipo_e10_c),2,'0') tipo_e10_c, e10_c, coord_x as coorx, to_char(coord_y) as coory, descrubic, sare_st estatus_punteo, e12, ");
...
//join tr_etq_val ue on ue.id_ue=pre.id_ue
        ...
        sql.append(" FROM ").append(esquemaPos).append(".VW_PUNTEO_SARE");
        if (origen == 1) {
            sql.append(" where sare_st='10' and id_ue = '").append(id_ue).append("'");
        } else {
            sql.append(" where id_ue = '").append(id_ue).append("'");
        }
        if (!(ce == null)) {
            if (!ce.equals("00") && !ce.equals("99")) {
                sql.append("and cestatal = '").append(ce).append("'");
            }
        }
        return sql;
    }

    private StringBuilder filtrarSqlMasivoOtros(String ce, String esquemaPos, String id_ue, int origen, String tramo) {
        String esquemaOcl = esquemaPos;
        StringBuilder sql = new StringBuilder();
//        sql.append("SELECT to_char(id_ue) as id_ue, e03, e04, e05, e06, e07, e08, e09, lpad(to_char(tipo_e10),2,'0') tipo_e10, e10, e11, TRIM(e11a) as e11a, lpad(to_char(tipo_e14),2,'0') tipo_e14, e14, lpad(to_char(tipo_e10_a),2,'0') tipo_e10_a, e10_a, ");
//        sql.append("lpad(to_char(tipo_e10_b),2,'0') tipo_e10_b, e10_b, lpad(to_char(tipo_e10_c),2,'0') tipo_e10_c, e10_c, coord_x as coorx, to_char(coord_y) as coory, descrubic, sare_st estatus_punteo, e12, ");
